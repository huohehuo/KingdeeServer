 ...
 */
@WebServlet(urlPatterns = "/feedback_delete")
public class feedback_delete extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(paramter!=null&&!paramter.equals("")){
            try {
                    ...
                    SQL = "DELETE  FROM FeedBackOfWeb WHERE id = ?";
                    sta = conn.prepareStatement(SQL);
                    sta.setString(1,paramter);
                    int i = sta.executeUpdate();
                    if(i>0){
                        SQL="删除成功";
//                        request.setAttribute("tips", SQL);
