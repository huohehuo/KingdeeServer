            ...
            try {
                ...
                switch (pushDownListRequestBean.id) {
                    ...
                    case 8://领料单验货
                        ...
                        break;
                    case 9://生产单下推产品入库
                        SQL = "select FInterID,FBillNo,FWorkShop as FDeptID ,'' as FSupplyID,'' as FMangerID,'' as FEmpID,t2.FName,FPlanCommitDate as FDate,row_number() over (order by t1.FBillNo desc) as row_num from ICMO t1 left join t_Department t2 on t1.FWorkShop = t2.FItemID where  FAuxQty-FAuxCommitQty>0 and FStatus in(1,2)" + condition + "order by t1.FBillNo desc";
                        break;
                    case 10://生产汇报单
                ...
                if (rs.next()) {
                    ...
                    switch (pushDownListRequestBean.id){
                        ...
                        case 8:
                            ...
                            break;
                        case 9:
                            SQL = "select FInterID,FBillNo,FWorkShop,t2.FName as FDepartmentName,t1.FItemID,t1.FUnitID,FPlanCommitDate,FPlanFinishDate,(FAuxQty-FAuxCommitQty) as FAuxQty,(FAuxQtyForItem+FAuxQtyScrap) as FAux,'0' as FQtying,'0' as FAuxPrice,t3.FName,t3.FModel,0 as FEntryID,t3.FNumber from ICMO t1 left join t_Department t2 on t1.FWorkShop = t2.FItemID left join t_ICItem t3 on t1.FItemID=t3.FItemID where  FAuxQty-FAuxCommitQty>0 and FStatus in(1,2) and t1.FInterID="+pushDownListBean.FInterID;
                            break;
                        case 10:
