if (exists (select * from sys.objects where name = 'proc_Delivery'))
    drop proc proc_Delivery
go
create proc proc_Delivery
(
...
set @FCustID =dbo.getString(@mainStr,'|',4)   --购货单位id 
...
exec GetICMaxNum 'SEOutStock',@FInterID output,1,@FBillerID --得到@FInterID
------------------------------------------------------------得到编号
 set @FBillNo ='' 
 exec proc_GetICBillNo 83, @FBillNo out 
-----------------------------------------------------------得到编号
set @FCurrencyID=1 --币别
set @FCheckDate=null --审核时间  
set @FExplanation='' --备注

INSERT INTO SEOutStock(FInterID,FBillNo,FBrNo,FTranType,FCancellation,FStatus,Fdate,FCustID,FSalType,FCheckDate,FManagerID,
FDeptID,FEmpID,FBillerID,FSettleID,FCurrencyID,FExchangeRate,FMultiCheckDate1,FMultiCheckDate2,FMultiCheckDate3,FMultiCheckDate4,
FMultiCheckDate5,FMultiCheckDate6,FRelateBrID,FSelTranType,FFetchAdd,FExplanation,FAreaPS,FManageType)
VALUES (@FInterID,@FBillNo,'0',83,0,0,@Fdate,@FCustID,101,@FCheckDate,@FManagerID,@FDeptID,@FEmpID,@FBillerID,1,1,1,
Null,Null,Null,Null,Null,Null,0,81,'','',20302,0)
...
declare @FEntryID varchar(20),       --新的明细序号
        ...
        @FComplexQty varchar(50),  --基本数量+单位
        @FStockQtyOnlyForShow float , --单位库存
	...
	select @FExchangeRate=isnull(FExchangeRate,1),@FSourceBillNo=FBillNo from SEOrder where FInterID=@FSourceInterId --下推的单据编号
	...
	select @FCoefficient=isnull(FCoefficient,1),@FComplexQty=convert(varchar(50),@Fauxqty)+FName from t_MeasureUnit where FMeasureUnitID=@FUnitID --单位换算率
	set @FQtyMust=@FAuxQtyMust*@FCoefficient --基本单位可验收的数量 
	select @FStockQtyOnlyForShow=FQty from ICInventory where FItemID=@FItemID and FStockID=@FDCStockID and FStockPlaceID=@FDCSPID and FBatchNo=@FBatchNo --库存
	if(@FStockQtyOnlyForShow is null or @FStockQtyOnlyForShow='')
	set @FStockQtyOnlyForShow=0
	else set @FStockQtyOnlyForShow=@FStockQtyOnlyForShow/@FCoefficient
 ...
 INSERT INTO SEOutStockEntry (FInterID,FEntryID,FBrNo,FMapNumber,FItemID,FMapName,FAuxPropID,FQty,FBatchNo,FUnitID,Fauxqty,
 FStockQtyOnlyForShow,FSecCoefficient,FSecQty,Fauxprice,Famount,Fnote,FSourceBillNo,FSourceTranType,FSourceInterId,
 FSourceEntryID,FContractBillNo,FContractInterID,FContractEntryID,FOrderBillNo,FOrderInterID,FOrderEntryID,FFetchDate,
 FStockID,FStdAmount,FPlanMode,FMTONo) 
 VALUES (@FInterID,@FEntryID,'0','',@FItemID,'',0,@FQty,@FBatchNo,@FUnitID,@Fauxqty,@FStockQtyOnlyForShow,0
 ,0,@Fauxprice,@Famount,'',@FSourceBillNo,81,@FSourceInterId,@FSourceEntryID,'',0,0,@FSourceBillNo,@FSourceInterId,@FSourceEntryID,
@Fdate,@FDCStockID,@FAmount,14036,'')
end
set @detailqty=@detailqty+1
end  
EXEC p_UpdateBillRelateData 83,@FInterID,'SEOutStock','SEOutStockEntry'
...
Update t
Set t.FStatus =Case When (SELECT COUNT(1) FROM SEOrderEntry WHERE (FCommitQty>0 OR ( ISNULL(FMRPClosed,0) = 1 And FQty <> 0)) AND FInterID IN(@FInterID))=0 Then 1 When EXISTS(SELECT 1 FROM SEOrderEntry te WHERE ISNULL(te.FMRPAutoClosed,1)=1 AND  FCommitQty < FQty  AND te.FInterID IN(@FInterID)) Then 2 Else 3 End
,t.FClosed =Case When EXISTS(SELECT 1 FROM SEOrderEntry te WHERE ISNULL(te.FMRPAutoClosed,1)=1 AND  FCommitQty < FQty  AND te.FInterID IN(@FInterID)) Then 0 Else 1 End
From SEOrder t
WHERE t.FInterID IN(@FInterID)
IF EXISTS (SELECT 1 FROM ICBillRelations_Sale WHERE FBillType = 83 AND FBillID=@FInterID)
BEGIN
    ...
    WHERE t3.FBillType=83 AND t3.FBillID=@FInterID
END
ELSE
BEGIN
    ...
    WHERE t1.FTranType=83 AND t1.FInterID=@FInterID AND t2.FSourceInterID>0
END

if not exists(  select   1  from SEOutStockEntry where FInterID=@FInterID)
begin
    delete  SEOutStock where FInterID=@FInterID
	goto error1
end 
commit tran 
return;
