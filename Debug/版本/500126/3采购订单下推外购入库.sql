...
FUpStockWhenSave,FVchInterID,FROB,FHookStatus,Fdate,FSupplyID,FCheckDate,FFManagerID,
FSManagerID,FBillerID,FPOStyle,FMultiCheckDate1,FMultiCheckDate2,FMultiCheckDate3,
FMultiCheckDate4,FMultiCheckDate5,FMultiCheckDate6,FRelateBrID,FPOOrdBillNo,
FOrgBillInterID,FSelTranType,FBrID,FExplanation,FDeptID,FManagerID,FEmpID,
FCussentAcctID,FManageType,FSettleDate,FPrintCount) 
VALUES (@FInterID,@FBillNo,'0',1,0,0,@value,0,1,0,@Fdate,@FSupplyID,Null,@FFManagerID,@FSManagerID,
@FBillerID,@FPOStyle,Null,Null,Null,Null,Null,Null,0,'',0,71,0,@FExplanation,@FDeptID,@FManagerID,
@FEmpID,0,0,@FSettleDate,0)
...
select @FExchangeRate=isnull(FExchangeRate,1),@FSourceBillNo=FBillNo from POOrder where FInterID=@FSourceInterId --下推的单据编号
   ...
   FAuxPropID,FBatchNo,FQtyMust,FQty,FUnitID,FAuxQtyMust,Fauxqty,FSecCoefficient,FSecQty,FAuxPlanPrice,FPlanAmount,Fauxprice,Famount,Fnote,FKFDate,FKFPeriod,
   FPeriodDate,FDCStockID,FDCSPID,FOrgBillEntryID,FSNListID,FSourceBillNo,
   ...
   FAllHookAmount,FCurrentHookQTY,FCurrentHookAmount,FAuxQtyInvoice,FSecInvoiceQty,
   FQtyInvoice,FPlanMode,FMTONo) 
VALUES (@FInterID,@FEntryID,'0','','',@FItemID,0,@FBatchNo,@FQtyMust,@FQty,@FUnitID,@FAuxQtyMust,
@Fauxqty,@FSecCoefficient,@FSecQty,@FAuxPlanPrice,@FPlanAmount,@Fauxprice,@Famount,'',Null,0,Null,@FDCStockID,@FDCSPID,
0,0,@FSourceBillNo,71,@FSourceInterId,@FSourceEntryID,'',0,0,@FSourceBillNo,@FSourceInterId,@FSourceEntryID,0,0,0,0,0,0,0,14036,'')
...
Update t
Set t.FStatus =Case When (SELECT COUNT(1) FROM POOrderEntry WHERE (FCommitQty>0 OR ( ISNULL(FMRPClosed,0) = 1 And FQty <> 0)) AND FInterID IN(@FInterID))=0 Then 1 When EXISTS(SELECT 1 FROM POOrderEntry te WHERE ISNULL(te.FMRPAutoClosed,1)=1 AND  FCommitQty < FQty  AND te.FInterID IN(@FInterID)) Then 2 Else 3 End
,t.FClosed =Case When EXISTS(SELECT 1 FROM POOrderEntry te WHERE ISNULL(te.FMRPAutoClosed,1)=1 AND  FCommitQty < FQty  AND te.FInterID IN(@FInterID)) Then 0 Else 1 End
From POOrder t
