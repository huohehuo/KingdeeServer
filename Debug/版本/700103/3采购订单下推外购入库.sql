...
FExplanation,FDeptID,FManagerID,FEmpID,FCussentAcctID,FManageType,FSettleDate,FPrintCount) 
SELECT @FInterID,@FBillNo,'0',1,0,0,@value,1,0,@Fdate,@FSupplyID,@FCheckDate,@FFManagerID,@FSManagerID,@FBillerID,@FPOStyle,Null,Null,Null,Null,Null,Null,'',0,0,71,0,@FExplanation,@FDeptID,@FManagerID,@FEmpID,0,0,@FSettleDate,0
...
select @FExchangeRate=isnull(FExchangeRate,1),@FSourceBillNo=FBillNo from POOrder where FInterID=@FSourceInterId --下推的单据编号
  ...
  FCurrentHookQTY,FCurrentHookAmount,FPlanMode,FMTONo)  
  SELECT @FInterID,@FEntryID,'0','','',@FItemID,0,@FBatchNo,@FQtyMust,@FQty,@FUnitID,@FAuxQtyMust,@Fauxqty,@FSecCoefficient,@FSecQty,@FAuxPlanPrice,@FPlanAmount,@Fauxprice,@Famount,'',Null,0,Null,@FDCStockID,@FDCSPID,0,0,@FSourceBillNo,71,@FSourceInterId,@FSourceEntryID,'',0,0,@FSourceBillNo,@FSourceInterId,@FSourceEntryID,0,0,0,0,14036,''  
