...
set @FFetchAdd=dbo.getString(@mainStr,'|',11) --交货地点 
...
select @Fnumber=a.FProjectVal from t_billcoderule a
 ...
 where a.fbilltypeid = '21' and  a.FProjectID=3 order by a.FClassIndex
 --流水号长度
...
---更新流水号
 ...
 left join t_option e on a.fprojectid=e.fprojectid and a.fformatindex=e.fid
 Left OUter join t_checkproject b on a.fbilltype=b.fbilltypeid and a.fprojectval=b.ffield
 where a.fbilltypeid = '21' and a.FProjectID=3  ---更新单号
...
select @FClassIndex= COUNT(1) from t_billcoderule a
 ...
 left join t_option b on a.fprojectid=b.fprojectid and a.fformatindex=b.fid
 left join t_checkproject c on a.fbilltype = c.fbilltypeid and a.fprojectval = c.ffield
 where a.fbilltypeid = 21 and a.FClassIndex=@Index
 if(@FProjectID='1')
 begin
 --编号前缀    
select @Fprefix=a.FProjectVal from t_billcoderule a
 left join t_option e on a.fprojectid=e.fprojectid and a.fformatindex=e.fid
 Left OUter join t_checkproject b on a.fbilltype=b.fbilltypeid and a.fprojectval=b.ffield
 where a.fbilltypeid = '21' and  a.FProjectID=1 and a.FClassIndex=@Index order by a.FClassIndex
 ...
 left join t_option e on a.fprojectid=e.fprojectid and a.fformatindex=e.fid
 Left OUter join t_checkproject b on a.fbilltype=b.fbilltypeid and a.fprojectval=b.ffield
 where a.fbilltypeid = '21' and  a.FProjectID=2 and a.FClassIndex=@Index order by a.FClassIndex
...
INSERT INTO ICStockBill(FInterID,FBillNo,FBrNo,FTranType,FCancellation,FStatus,FUpStockWhenSave,FVchInterID,FROB,
FHookStatus,Fdate,FSupplyID,FSaleStyle,FCheckDate,FFManagerID,FDeptID,FBillerID,FOrgBillInterID,FSelTranType,FFetchAdd,
FExplanation,FManagerID,FEmpID,FSManagerID,FMultiCheckLevel1,FHolisticDiscountRate,FMultiCheckDate1,FMultiCheckLevel2,
...
FMultiCheckLevel6,FMultiCheckDate6,FRelateBrID,FMarketingStyle,FBrID,FVIPCardID,FVIPScore,FPOSName,FWorkShiftId) 
VALUES (@FInterID,@FBillNo,'0',21,0,0,@value,0,1,0,@Fdate,@FSupplyID,@FSaleStyle,@FCheckDate,@FFManagerID,@FDeptID,@FBillerID,0,81,@FFetchAdd,@FExplanation,@FManagerID,@FEmpID,@FSManagerID,Null,0,Null,Null,Null,Null,Null,Null,Null,Null,Null,Null,Null,0,12530,0,0,0,'',0)  
...
INSERT INTO ICStockBillEntry (FInterID,FEntryID,FBrNo,FItemID,FMapNumber,FMapName,FDCStockID,FAuxPropID,FBatchNo,
FCustItemNo,FCustItemName,FQty,FUnitID,FAuxQtyMust,Fauxqty,Fauxprice,Famount,Fnote,FSecCoefficient,FKFDate,FKFPeriod,
FSecQty,FAuxPlanPrice,FPeriodDate,FConsignPrice,FPlanAmount,FDiscountRate,FDiscountAmount,FConsignAmount,FOrgBillEntryID,
FSourceBillNo,FSourceTranType,FSourceInterId,FSourceEntryID,FOrderBillNo,FDVStockID,FOrderInterID,FDCSPID,FOrderEntryID,
FQtyMust,FAuxQtyInvoice,FQtyInvoice,FSNListID,FContractBillNo,FContractInterID,FContractEntryID,FAllHookQTY,FCurrentHookQTY,FSepcialSaleId) 
VALUES (@FInterID,@FEntryID,'0',@FItemID,'','',@FDCStockID,0,@FBatchNo,'','',@FQty,@FUnitID,@FAuxQtyMust,@Fauxqty,0,0,'',
0,Null,0,0,@FAuxPlanPrice,Null,@FConsignPrice,@FPlanAmount,0,0,@FConsignAmount,0,@FSourceBillNo,81,@FSourceInterId,
@FSourceEntryID,@FSourceBillNo,0,@FSourceInterId,@FDCSPID,@FSourceEntryID,@FQtyMust,0,0,0,'',0,0,0,0,0) 
...
Case when (select count(*) from SEOrderEntry M inner join t_Icitem T on M.FItemID=T.FItemID  where M.FInterID=t1.FInterID and Round(FCommitQty,T.FQtyDecimal)<>0)=0 then 1 
when (select count(*) from SEOrderEntry M inner join t_Icitem T on M.FItemID=T.FItemID  where M.FInterID=t1.FInterID and Round(abs(FQty),T.FQtyDecimal)>Round(abs(FCommitQty),T.FQtyDecimal))>0 then 2 
else 3 End 
From SEOrder t1, ICStockBillEntry t2 
