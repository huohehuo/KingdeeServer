...
declare @FEntryID varchar(20),       --新的明细序号
	...
	set @FSCStockID=dbo.getString(@detailStr1,'|',@detailcount*@detailIndex+5) --调出仓库
	set @FSCSPID=dbo.getString(@detailStr1,'|',@detailcount*@detailIndex+6) --调出仓位
	...
	set @FDCStockID=dbo.getString(@detailStr1,'|',@detailcount*@detailIndex+10) --调入仓库id
	set @FDCSPID=dbo.getString(@detailStr1,'|',@detailcount*@detailIndex+11) --调入仓位id  
...
FUnitID,Fauxqty,FSecCoefficient,FSecQty,FAuxPlanPrice,FPlanAmount,Fauxprice,Famount,FAuxPriceRef,FAmtRef,Fnote,
...
FSourceEntryID,FICMOBillNo,FICMOInterID,FPPBomEntryID,FOrderBillNo,FOrderInterID,FOrderEntryID,FPlanMode,FMTONo) VALUES (@FInterID,@FEntryID,'0',@FItemID,0,@FBatchNo,@FQty,@FUnitID,@Fauxqty,
0,0,0,0,0,0,0,0,'',Null,0,Null,@FSCStockID,@FSCSPID,@FDCStockID,@FDCSPID,0,@FSourceBillNo,83,@FSourceInterId,@FSourceEntryID,
'',0,0,@FOrderBillNo,@FOrderInterID,@FOrderEntryID,14036,'') --FOrderBillNo,FOrderInterID,FOrderEntryID= '',0,0
end
